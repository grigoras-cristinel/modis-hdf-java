 ...
 * 
 * It is assumed that the data provided are within valie ranges (i.e. latitude
...
public class SunriseSunset {
		...
		try {
			...
			dfJ = -Math.floor(7.0 // SUNUP used INT, not floor
					* (Math.floor((iMonth + 9.0) / 12.0) + iYear) / 4.0)
					...
					+ Math.floor(iMonth * 275.0 / 9.0) + iDay + 1721027.0
					+ iYear * 367.0;
			if (bGregorian) {
				// SUNUP.BAS 1230
				if ((iMonth - 9.0) < 0.0)
				...
				dfJ3 = -Math.floor((Math.floor(Math.floor(iYear + iSign
			...
			for (iCount = 0; iCount <= 1; iCount++) // Loop thru only twice
			{
				...
				dfVV = 0.39785 * Math.sin(dfLL) - 0.01000
						...
						* Math.sin(dfLL + dfGG) - 0.00021 * Math.sin(dfLL)
						* dfTT;
				dfUU = 1 - 0.03349 * Math.cos(dfGG) - 0.00014
						* Math.cos(dfLL * 2.0) + 0.00008 * Math.cos(dfLL);
				dfWW = -0.00010 - 0.04129 * Math.sin(dfLL * 2.0) + 0.03211
						* Math.sin(dfGG) - 0.00104
			...
			for (iCount = 0; iCount < 24; iCount++) // SUNUP.BAS 210
			{
				...
				dfV2 = dfSinLat * Math.sin(dfD2) + dfCosLat * Math.cos(dfD2)
						* Math.cos(dfH2) - Math.cos(dfZenith); // SUNUP.BAS 570
				...
				dfV1 = dfSinLat * Math.sin(dfD1) + dfCosLat * Math.cos(dfD1)
						* Math.cos(dfH1) - Math.cos(dfZenith); // SUNUP.BAS 590
			...
			if (bSunsetToday) {
				dateSunset = dfmtDateTime.parse(iDay + " " + iMonth + " "
						+ iYear + " " + (int) dfHourSet + ":" + (int) dfMinSet
						+ " GMT");
			}
		} // end of try
			// Catch errors
	...
	public boolean isDaytime() {
		...
		if (bSunriseToday && bSunsetToday) // sunrise and sunset
		{
			if (dateSunrise.before(dateSunset)) // sunrise < sunset
			{
				if ((dateInput.after(dateSunrise) || dateInput
						.equals(dateSunrise)) && dateInput.before(dateSunset))
					bDaytime = true;
				else
					bDaytime = false;
			...
			{
				if ((dateInput.after(dateSunrise) || dateInput
						.equals(dateSunrise)) || // use OR rather than AND
		...
		} else if (bSunsetToday) // sunset but no sunrise
		{
			...
			bDaytime = false; // this should never execute
		return (bDaytime);
	}
} // end of class
/*-----------------------------------------------------------------------------
