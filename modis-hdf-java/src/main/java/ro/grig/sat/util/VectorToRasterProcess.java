 ...
 * <p>
 * A feature attribute is specified from which to extract the numeric values
 * that will be written to the output grid coverage. At present only int or
 * float values are written to the output grid coverage. If the attribute is of
 * type Long it will be coerced to int values and a warning will be logged.
 * Similarly if the attribute is of type Double it will be coerced to float and
 * a warning logged.
 *
 ...
 *
 * @source $URL:
 *         http://svn.osgeo.org/geotools/tags/2.7-M3/modules/unsupported/process
 *         /src/main/java/org/geotools/process/raster/VectorToRasterProcess.java
 *         $
 */
public class VectorToRasterProcess extends AbstractFeatureCollectionProcess {
    ...
    * <p>
    * The process interface is useful for advertising functionality to dynamic
    * applications, but for 'hands on' coding this method is much more
    * convenient than working via the
    * {@linkplain org.geotools.process.Process#execute }.
    *
    * @param features
    *           the feature collection to be (wholly or partially) rasterized
    *
    * @param attribute
    *           source of values for the output grid: either a {@code String}
    *           for the name of a numeric feature property or an
    *           {@code org.opengis.filter.expression.Expression} that evaluates
    *           to a numeric value
    *
    * @param gridWidthInCells
    *           width (cell) of the output raster
    *
    * @param gridHeightInCells
    *           height (cell) of the output raster
    *
    * @param bounds
    *           bounds (world coordinates) of the output raster
    *
    * @param covName
    *           a name for the output raster
    *
    * @param monitor
    *           an optional {@code ProgressListener} (may be {@code null}
    *
    * @return a new grid coverage
    *
    * @throws org.geotools.process.raster.VectorToRasterException
    */
   public static GridCoverage2D process(SimpleFeatureCollection features, Object attribute, Dimension gridDim,
         Envelope bounds, String covName, ProgressListener monitor) throws VectorToRasterException {
   ...
   /**
    * Retrieves the input parameters from the supplied Map, conducts some basic
    * checking, and then carries out the vector to raster conversion.
    *
    * @param input
    *           input parameters from those defined in
    *           {@linkplain VectorToRasterFactory}
    *
    * @param monitor
    ...
    *
    * @throws org.geotools.process.raster.VectorToRasterException
    *            if unable to rasterize the features as requested
    *
    * @see VectorToRasterFactory#getResultInfo(java.util.Map)
    */
   public Map<String, Object> execute(Map<String, Object> input, ProgressListener monitor)
         throws VectorToRasterException {
      SimpleFeatureCollection features = (SimpleFeatureCollection) input
            .get(AbstractFeatureCollectionProcessFactory.FEATURES.key);
   ...
   /**
    * This method is called by {@linkplain #execute} to rasterize an individual
    * feature.
    *
    * @param feature
   ...
   private GridCoverage2D convert(SimpleFeatureCollection features, Object attribute, Dimension gridDim,
         Envelope bounds, String covName, ProgressListener monitor) throws VectorToRasterException {
      if (monitor == null) {
   ...
   private void initialize(SimpleFeatureCollection features, Envelope bounds, Object attribute, Dimension gridDim)
         throws VectorToRasterException {
    ...
    * @throws org.geotools.process.raster.VectorToRasterException
    */
   private void setBounds(SimpleFeatureCollection features, Envelope bounds, Dimension gridDim)
         throws VectorToRasterException {
      ...
      if (bounds != null) {
         ...
         /*
          * The bounds arg was null - interpreted as set bounds to those of the
          * FeatureCollection
          */
   ...
   /**
    * Create the tiled image and the associated graphics object that we will be
    * used to draw the vector features into a raster.
    * <p>
    * Note, the graphics objects will be an instance of TiledImageGraphics which
    * is a sub-class of Graphics2D.
    */
   ...
   /**
    * Takes the 4-band ARGB image that we have been drawing into and converts it
    * to a single-band image.
    *
    * @todo There is probably a much easier / faster way to do this that still
    *       takes advantage of image tiling (?)
    */
   ...
   /**
    * Takes the 4-band ARGB image that we have been drawing into and converts it
    * to a single-band int image.
    */
   private void flattenImageToInt() {
      int numXTiles = image.getNumXTiles();
      int numYTiles = image.getNumYTiles();
      SampleModel sm = RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_INT, image.getWidth(),
            image.getHeight(), 1);
      TiledImage destImage = new TiledImage(0, 0, image.getWidth(), image.getHeight(), 0, 0, sm,
            new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), false, false, Transparency.OPAQUE,
                  DataBuffer.TYPE_INT));
      for (int yt = 0; yt < numYTiles; yt++) {
         for (int xt = 0; xt < numXTiles; xt++) {
            ...
            srcTile
                  .getDataElements(srcTile.getMinX(), srcTile.getMinY(), srcTile.getWidth(), srcTile.getHeight(), data);
   ...
   /**
    * Takes the 4-band ARGB image that we have been drawing into and converts it
    * to a single-band float image
    */
   private void flattenImageToFloat() {
      int numXTiles = image.getNumXTiles();
      int numYTiles = image.getNumYTiles();
      SampleModel sm = RasterFactory.createPixelInterleavedSampleModel(DataBuffer.TYPE_FLOAT, image.getWidth(),
            image.getHeight(), 1);
      TiledImage destImage = new TiledImage(0, 0, image.getWidth(), image.getHeight(), 0, 0, sm,
            new ComponentColorModel(ColorSpace.getInstance(ColorSpace.CS_GRAY), false, false, Transparency.OPAQUE,
