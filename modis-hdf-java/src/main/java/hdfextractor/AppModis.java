...
/**
 * Aplicatie care extrage ozonul din HDF in coordonatele de statii date si le
 * face intr-un folder.
 * 
 */
public class AppModis {
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		...
		for (int j = 0; j < anii.length; j++) {
			...
			for (int k = 0; k < sateliti.length; k++) {
				...
				String inputPath = "C:/DATE-Satelit/" + satelit + "07_L2_"
						+ anul + "/";
				...
				for (String siteName : statii.keySet()) {
					File fileOutput = new File(inputPath + "out/ozon_"
							+ satelit + "07_" + anul + "_" + siteName + ".txt");
					System.out.println("Output file:" + fileOutput);
					try {
						PrintWriter pw = new PrintWriter(fileOutput);
						pw.println(OzonLineTO.longHeader());
						...
						for (String siteName : statii.keySet()) {
							if (cfile.hasPosition(siteName)) {
								...
								if (ozonDobson != null) {
									...
									Double ozlasol10 = calc1.calculeazaU(luna,
											ozonDobson, 10d);
									Double ozlasol2 = calc1.calculeazaU(luna,
											ozonDobson, 15d);
									Double ozlasol3 = calc1.calculeazaU(luna,
											ozonDobson, 20d);
									double dif2 = ozonDobson - ozlasol2;
									double dif3 = ozonDobson - ozlasol3;
									double v2_ugm3 = dif2 * 2.69e16 * mukg * 48
											* 1e9;
									double v3_ugm3 = dif3 * 2.69e16 * mukg * 48
											* 1e9;
