...
public class AppModis06 {
	...
	public static void main(String[] args) {
		...
		File bkupFisereRulate = new File("d:/grig/temp/historyProcesare_06.dmp");
		...
		String runPath = ConfigurationLocalComputer.CALE_DATE_SATELIT
				+ "/extras/" + AppModis06.class.getSimpleName() + "/";
		String runPathProgress = ConfigurationLocalComputer.CALE_DATE_SATELIT
				+ "/extras/" + AppModis06.class.getSimpleName() + "R1/";
		...
		for (int j = 0; j < anii.length; j++) {
			...
			for (int k = 0; k < sateliti.length; k++) {
				...
				String inputPath = ConfigurationLocalComputer.CALE_DATE_SATELIT
						+ "MOD06";
				String outputPath = runPath + satelit + "06_" + anul + "/";
					...
					for (int i = 0; i < deCitit.length; i++) {
						...
						for (String siteName : statii.keySet()) {
							...
							if (cfile.hasPosition(siteName)) {
								...
								System.out
										.print(" nu gasesc pozitia in acest fisier");
